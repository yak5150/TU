...
D_c_i = 1*0.0254; % m
D_c_o = 1.125*0.0254; % m

D_h_i = 1.5*0.0254; % m
D_h_o = 1.625*0.0254; % m
...
rho = 1; %g/mL, density of water

k_cu = 52; % W/m · K

thermo_prop = [273.15 0.00611 1.000 206.3 2502 4.217 1.854 1750 8.02 569 18.2 12.99 0.815 75.5 68.05 273.15; 275 0.00697 1.000 181.7 2497 4.211 1.855 1652 8.09 574 18.3 12.22 0.817 75.3 32.74 275; 280 0.00990 1.000 130.4 2485 4.198 1.858 1422 8.29 582 18.6 10.26 0.825 74.8 46.04 280; 285 0.01387 1.000 99.4 2473 4.189 1.861 1225 8.49 590 18.9 8.81 0.833 74.3 114.1 285; 290 0.01917 1.001 69.7 2461 4.184 1.864 1080 8.69 598 19.3 7.56 0.841 73.7 174.0 290; 295 0.02617 1.002 51.94 2449 4.181 1.868 959 8.89 606 19.5 6.62 0.849 72.7 227.5 295; 300 0.03531 1.003 39.13 2438 4.179 1.872 855 9.09 613 19.6 5.83 0.857 71.7 276.1 300; 305 0.04712 1.005 29.74 2426 4.178 1.877 769 9.29 620 20.1 5.20 0.865 70.9 320.6 305; 310 0.06221 1.007 22.93 2414 4.178 1.882 695 9.49 628 20.4 4.62 0.873 70.0 361.9 310; 315 0.08132 1.009 17.82 2402 4.179 1.888 631 9.69 634 20.7 4.16 0.883 69.2 400.4 315; 320 0.1053 1.011 13.98 2390 4.180 1.895 577 9.89 640 21.0 3.77 0.894 68.3 436.7 320; 325 0.1351 1.013 11.06 2378 4.182 1.903 528 10.09 645 21.3 3.42 0.901 67.5 471.2 325; 330 0.1719 1.016 8.82 2366 4.184 1.911 489 10.29 650 21.7 3.15 0.908 66.6 504.0 330; 335 0.2167 1.018 7.09 2354 4.186 1.920 453 10.49 656 22.0 2.88 0.916 65.8 535.5 335; 340 0.2713 1.021 5.74 2342 4.188 1.930 420 10.69 660 22.3 2.66 0.925 64.9 566.0 340; 345 0.3372 1.024 4.683 2329 4.191 1.941 389 10.89 664 22.6 2.45 0.933 64.1 595.4 345];

% Linear Interpolation Values
T_c_low = 0; T_c_up = 0; mu_c_low = 0; mu_c_up = 0; k_c_low = 0; k_c_up = 0; Pr_c_low = 0; Pr_c_up = 0; C_c_up = 0; C_c_low = 0;
mu_h_low = 0; mu_h_up = 0; k_h_low = 0; k_h_up = 0; Pr_h_low = 0; Pr_h_up = 0; C_h_up = 0; C_h_low = 0;
...
C_c = (((TcK-T_c_up)*(C_c_low-C_c_up))/(T_c_low-T_c_up))+C_c_up; % kJ/kg · K
...
C_h = (((ThK-T_h_up)*(C_h_low-C_h_up))/(T_h_low-T_h_up))+C_h_up; % kJ/kg · K

% Calculated Constants

% for air, Table A.5 of ISO 7730 outlines a maximum mean air velocity of
% 0.12 and 0.1 m/s in summer and winter respectively for offices. I assume
% it is winter since that is when it will be tested so I will use 0.1 m/s.
% Also, assuming 25 C

%%%%%%% I think this is wrong, 1) is it D or length 2) is the area for hot
%%%%%%% the area of inner - outer 
ReD_c = (m_c * D_c_i)/((pi*D_c_i^2/4)*mu_c);
% ReD_h = (m_h * D_h_i)/((pi*Do^2/4)*mu_h);
ReD_h = (m_h * D_h_i)/(pi*(D_h_i^2-D_c_i^2)*mu_h/4); % Using the actual area of the hot fluid for this calculation
Re_air = (1.1700321*0.1*L)/(183.675*10^-7); % Using length Re # bcs I am assuming air is going across the pipe on the outside
...
NuD_air = 0.023*(Re_air^0.8)*(Pr_air^0.4); % air is getting slightly heated

h_c = (k_c*NuD_c)/D_c_i;
h_h = (k_h*NuD_h)/D_h_i;
h_air = (k_air*NuD_air)/D_h_o;
...
U = (A_o*(R_c_conv+R_i_cond+R_h_conv+R_o_cond+R_air_conv))^-1; 
...
fprintf('\n Parallel Flow \n')
disp(['Heat Transfer Rate: ',num2str(q_p)])
disp(['Final Hot Temperature: ',num2str(T_h_o_p)])
disp(['Final Cold Temperature: ',num2str(T_c_o_p)])

fprintf('\n Counter Flow \n')
disp(['Heat Transfer Rate: ',num2str(q_c)])
disp(['Final Hot Temperature: ',num2str(T_h_o_c)])
disp(['Final Cold Temperature: ',num2str(T_c_o_c)])